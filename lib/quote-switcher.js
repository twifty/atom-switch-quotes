...
export const QuoteUtil = {
  escapeChar (char, text, escapeChar = '\\') {
    let result = ''
    let escaped = false
  ...
  unEscapeChar (char, text, escapeChar = '\\') {
    let result = ''
    let escaped = false

    for (let i = 0; i < text.length; i++) {
      if (text[i] === escapeChar && !escaped) {
        escaped = true
        continue
      }

      if (escaped && text[i] !== char) {
        result += escapeChar
      }
...
export class QuoteSwitcher {
  ...
  static languageGeneric = [{
    ...
    adjust: {column: 1},
    apply: text => `"${QuoteUtil.escapeChar('\"', text)}"`,
    remove: text => QuoteUtil.unEscapeChar('\"', text.slice(1, -1))
  },{
