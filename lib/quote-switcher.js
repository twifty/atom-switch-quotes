...
export class QuoteSwitcher {
  ...
  setGrammarRules (language) {
    ...
    */
    ...
    try {
      let lower = language.replace(/ /g, '-').toLowerCase()
      ...
      this.log(`Error while oading language rules for "${language}"!`)
    }

    mergeRules(QuoteSwitcher.languageGeneric)

    const langConfig = this.config[Object.keys(this.config)
      .find(k => k.toLowerCase() === language.toLowerCase())
    ];

    if (langConfig) {
      this.log(`adding toggle rules for "${language}"`)
      for (rules of langConfig)
        set.addToggle(rules)
    } else {
  ...
  rotatePoint (method, cursor) {
    ...
    if (mutated) {
      if ('toggle' === method) {
        ...
        rules.rotate(mutated.rules, method)
  ...
  rotateRange (method, selection) {
    ...
    rules.rotate(mutated.rules, 'embeds')
  ...
  rotateSelections (method) {
    ...
    for (let c = selections.length - 1; c >= 0; --c) {
      ...
      if (method === 'toggle' || selected.start.isEqual(selected.end)) {
  ...
  applyWrapRules (mutated, cursor) {
    for (let rule of mutated.rules.reverse()) {
      mutated.text = rule.wrap(mutated.text)
    }

    if (mutated.text.isValid()) {
      return this.mutateRange(mutated.range, mutated.text, cursor)
    }
  }
  ...
  convertToRawText (point) {
    ...
    if (this.scopesIntersect('string', scopes)) {
      ...
      for (let rule of rules.filterScopes("rotate")) {
        ...
        if (!(scope in cache)) {
          if (typeof rule.scope === 'string') {
            ...
            const range = this.expandRange("string", point)
